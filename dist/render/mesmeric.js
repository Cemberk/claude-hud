...
function getAnimFrame(time, frameCount) {
 ...
 */
function getFaceLines(mode, time) {
 ...
 */
function computeWaveValue(x, y, time, params) {
 ...
 */
function valueToChar(value, palette) {
 ...
 */
function generateGrid(mode, time) {
 ...
 */
function overlayFace(grid, mode, time) {
 ...
 */
function addStatusBar(grid, mode, detail) {
...
function detectMesmericMode(ctx) {
 ...
 */
export function renderMesmericFull(ctx) {
 ...
 */
export function renderMesmericCompact(ctx) {
    const { mode, detail } = detectMesmericMode(ctx);
    const params = MODE_CONFIGS[mode];
    const time = Date.now() * params.velocity * 0.001;
 ...
 */
export function renderMesmericLine(ctx) {
 ...
 */
export function renderMesmericDemo(mode) {
    const params = MODE_CONFIGS[mode];
    const time = Date.now() * params.velocity * 0.001;
    const grid = generateGrid(mode, time);
    overlayFace(grid, mode, time);
...
//# sourceMappingURL=mesmeric.js.map